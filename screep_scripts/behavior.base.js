...
var expend_energy_to = {
	...
	transfer_nearest_creep_type : (creep_type_name) => (actor) => move_resource_action_nearest(actor, 'transfer', RESOURCE_ENERGY, FIND_MY_CREEPS,
			(creep) => creep.name.split(':')[0] == creep_type_name && creep.carry.energy < creep.carryCapacity),
	...
	repair_lowest_hit_type : (actor, struct_type) => move_action_on_target(actor, 'repair', get_target.lowest_hits(actor, FIND_STRUCTURES,
		(struct) => struct.structureType == struct_type && struct.hits < struct.maxHits)),
	attack_nearest_type : (actor, search_type) => move_action_nearest(actor, 'attack', search_type, (obj) => obj.owner.username != actor.owner.username),
	ranged_attack_nearest_type : (actor, search_type) => move_action_nearest(actor, 'rangedAttack', search_type,
		(obj) => obj.owner.username != actor.owner.username),
};
